...
Then run the installation command. Note that by default the example i presented here suggests the use of the ```checkip.amazonaws.com service``` to retrieve the VM's public IP when setting the "capsulePublicHost" value. Furthermore, a common name (CN) that's going to be used to generate the self-signed certificates for the tls configuration is given through the ```istio.tls.commonName``` variable. This implies that the user may provide a domain name pointing at the capsule instance instead, if available, with the ```capsulePublicHost``` and  ```istio.tls.commonName``` variables. A common scenario for capsule deployment is that of a private VM that is only be accessible through a private network. In this case, the IP that needs to be specified correspods to that of the VM itself. Please do note that this must also be the IP given when running the ansible playbook, if the environment has been set up this way.
```
export CAPSULE_PUB_IP=$(curl -s checkip.amazonaws.com); microk8s.helm install idea4rc-capsule idea4rc-helm-capsule/ --set capsulePublicHost=$CAPSULE_PUB_IP --set istio.tls.commonName=$CAPSULE_PUB_IP
```
...
## How to upgrade
> [!IMPORTANT]
> By design, the capsule should be destroyed an recreated every time the user wishes to ingest a different dataset. The facilities included in the Helm chart to facilitate the upgrade of the existing deployment do not represent a guarantee. Moreover, when upgrading between charts with major changes, the upgrade process could error out.

In order to upgrade an existing capsule, downloading the lastest release and upgading it with an ```helm upgrade``` using the relative configuration variables should suffice. However, this might not always be the case depending on a number of factors. Please note the this Helm chart leverages annotations to trigger the redeployment of the dependencies of the components that have been updated, hence no manual intervention is theoretically required. As a side effect, dependiencies could trigger the redeployment of the dependent pods - although, data persistency should be guaranteed for those deployments that leverage persisten volumes. 

Here's an example of the upgrade process:
